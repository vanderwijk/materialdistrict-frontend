...
### Opgelost (board detail, 04-06-2026) ✅
- **Board detail** — `GET /boards/{id}` + `/dashboard/boards/[id]` (zip FINAL).
...
**Status:** afgerond 31-07-2026 (frontend polish-ronde).

### ~~SL-UI-1 — Channel-pagina achtergrondkleur~~ ✅
`.channel-hero` gebruikt `var(--bg)` i.p.v. `var(--surface2)`.

### ~~SL-UI-2 — “Something broken”-modal (responsive + copy)~~ ✅
Trigger verborgen bij openen; panel bounded + safe-area op mobiel; copy:
“We include the page address automatically.”

### ~~SL-UI-3 — Stories/materials-blokken: 3 items over 2 kolommen~~ ✅
`.hp-main .grid-3` → 1 kolom onder 480px (wint van de 2-kolomsregel ≤1000px).

### ~~SL-UI-4 — Zoekpagina mobiel (toetsenbord, breadcrumb, breedte)~~ ✅
Breadcrumb zonder dubbel Home; zoekveld volle breedte op ≤560px; client
`SearchForm` blurt input bij submit zodat het toetsenbord dichtklapt.
