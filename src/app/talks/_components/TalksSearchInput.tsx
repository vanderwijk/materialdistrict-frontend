...
/**
 * TalksSearchInput — debounced zoek-input voor de talks-overzichtspage.
 *
 * Sessie 7. Parallel aan Articles/Brands/MaterialsSearchInput:
 *  - Initial value uit `?q=` (server-rendered).
 *  - Na 300ms stilte → `?q=` in de URL via router.push (useTransition).
 *  - Reset `?page=` bij wijziging.
 *  - Behoudt overige searchParams.
 *  - Clear-knop wist `?q=`.
 */
...
export interface TalksSearchInputProps {
  initialValue?: string
  placeholder?: string
  debounceMs?: number
}

export function TalksSearchInput({
  initialValue = '',
  placeholder = 'Search talks…',
  debounceMs = 300,
}: TalksSearchInputProps) {
  ...
  return (
    <div className="materials-search">
      <label className="u-visually-hidden" htmlFor="talks-search-input">
        {placeholder}
      </label>
